...
#define STREL_SIZE 5
// R must be a compile time constant for shared memory array sizes if used in template arguments, 
// usually kernel parameters like this are fine with macros.
// Ensure block/grid size calculations in main/kernelcall respect these.
...
int main(int argc, char** argv)
{
    ...
    {
        // 1. Denoise first (KNN) to match CPU pipeline
        // Use hardcoded params (k=3, h=40.0) as per project standard
        kernel_knn_host(device_dst, device_img, width, height, 3, 40.0);
        cudaMemcpy(out, device_dst, width * height * sizeof(Rgb), cudaMemcpyDeviceToHost);
        device_to_img(out, image); // Update 'image' with denoised version

        // 2. Continue with Edge Detect (on Denoised Image)
    ...
    // Ensure output directory exists provided by the user script (we assume 'output' dir exists)
    ...
    else
    {
        ...
        cv::imwrite(output_filename, grey_img);
        std::cout << "Saved output to " << output_filename << std::endl;
    }
    // cv::waitKey(0);
    grey_img.release();

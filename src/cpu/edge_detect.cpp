...
cv::Mat conv_with_mask(cv::Mat image, int conv_size)
{
    cv::Mat dst;
    cv::Mat tmp_image;
    double otsu_threshold = cv::threshold(image, dst, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
...
// Pixelizes the image
cv::Mat pixelize(cv::Mat image, int pix_size)
{
    auto res = image.clone();
    for (int y = 0; y < image.cols; y += pix_size)
    {
        for (int x = 0; x < image.rows; x += pix_size)
        {
            int b = 0;
            int g = 0;
            int r = 0;
            ...
            // Calculate average
            for (int j = y; j < y + pix_size && j < image.cols; j++)
            {
                for (int i = x; i < x + pix_size && i < image.rows; i++)
                {
            ...
            if (count > 0)
            {
                ...
                for (int j = y; j < y + pix_size && j < image.cols; j++)
                {
                    for (int i = x; i < x + pix_size && i < image.rows; i++)
                    {
                         res.at<cv::Vec3b>(i, j)[0] = b;
                         res.at<cv::Vec3b>(i, j)[1] = g;
                         res.at<cv::Vec3b>(i, j)[2] = r;
                    }
                }
            }
        }
